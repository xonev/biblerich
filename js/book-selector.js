...
void function () {
  var CHAPTER_COUNTS;
  window.BookSelector = BookSelector = function (super$) {
    extends$(BookSelector, super$);
    ...
    BookSelector.prototype.initialize = function (param$) {
      var $optgroup, book, cache$, options;
      {
        cache$ = param$;
        this.firstOption = cache$.firstOption;
        this.oldTestament = cache$.oldTestament;
        this.newTestament = cache$.newTestament;
      }
      this.updateChapters();
      if (this.oldTestament) {
        $optgroup = this.$el.find('.books optgroup[label="Old Testament"]');
        $optgroup.find('option').remove();
        options = [function (accum$) {
            for (var i$ = 0, length$ = this.oldTestament.length; i$ < length$; ++i$) {
              book = this.oldTestament[i$];
              accum$.push('<option>' + book + '</option>');
            }
            return accum$;
          }.call(this, [])];
      ...
      if (this.newTestament) {
        ...
        $optgroup.find('option').remove();
        options = [function (accum$1) {
            for (var i$1 = 0, length$1 = this.newTestament.length; i$1 < length$1; ++i$1) {
              book = this.newTestament[i$1];
              accum$1.push('<option>' + book + '</option>');
            }
            return accum$1;
          }.call(this, [])];
    ...
    BookSelector.prototype.onKeyup = function (event) {
      if (!(event.keyCode === 13))
        return;
      return this.go();
    };
    ...
    BookSelector.prototype.updateChapters = function () {
      ...
      if (this.firstOption)
        chapters = "\n        <optgroup label='Book'><option value='0'>" + this.firstOption + "</option></optgroup>\n        <optgroup label='Chapters'>" + chapters + '</optgroup>';
